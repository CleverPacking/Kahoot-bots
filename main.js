    ...
    document.getElementById("form").addEventListener("submit", (e) => {
      this.formHandler
        ...
        .then((data) => {
          ...
          this.socketHandler.addMessage(`📩 Response: ${data.response.text} - ${data.response.description}`, "info");
...
class SocketHandler {
  constructor() {
    this.socket = io("https://kahootbot.net", { path: "/api/socket" }); //io("https://kahootbot.net", { path: "/api/socket" });
  ...
  #registerListeners() {
    ...
    this.socket.on("botJoinFail", (data) => {
      this.addMessage(`⚠️ ${data.message}`, "warning");
    });

    // Bot failed to spawn
    this.socket.on("botSpawnError", (data) => {
      this.addMessage(`❌ Error ${data.code}: ${data.description}`, "error");
    });

    // Quiz started
    this.socket.on("quizStarted", (data) => {
      this.addMessage(`🏆 Quiz has started!`, "success");
    });

    // Question answered
    this.socket.on("questionAnswered", (data) => {
      this.addMessage(`📝 Bots answered ${data.answer}!`, "success");
    ...
    this.socket.on("questionStart", (data) => {
      ...
      // notify user to select answer
        ...
        this.addMessage("👀 New Question, answer now!"); // new question
      }
    });
  }
}
...
class FormHandler {
  ...
  async formSubmitHandler(event) {
    ...
    const name = this.name.value || "KahootBot.net";
    this.clearFields(); // clear fields

    // Prepare server data
    const clientId = localStorage.getItem("clientId");
    localStorage.setItem("clientId", clientId);
    const response = await fetch("https://kahootbot.net/api/init", {
      //fetch("https://kahootbot.net/api/init", {
      method: "POST",
      headers: {
